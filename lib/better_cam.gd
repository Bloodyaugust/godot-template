...
func _unhandled_input(event):
	if event is InputEventPanGesture:
		_target_zoom = (
			_target_zoom
			+ Vector2(event.delta.y * camera_zoom_speed, event.delta.y * camera_zoom_speed)
		)
	...
	if event is InputEventMouseButton:
		...
		if (
			...
			_target.global_position = _target.global_position.lerp(
				get_global_mouse_position(), (1 - (zoom.x / zoom_max)) * zoom_follow_scalar
			)
	...
	_target_zoom = Vector2(
		clamp(_target_zoom.x, zoom_min, zoom_max), clamp(_target_zoom.y, zoom_min, zoom_max)
	)
