...
func _diff_text(w1, w2, diff_col_pad = 10):
	...
	for key in w1:
		...
		to_return += str(
			str(prefix, key.capitalize()).rpad(WARN_VALUE_PRINT_POSITION, " "), diff_text, "\n"
		)
...
func _diff_changes_text(pre_settings):
	var orig_diff_text = _diff_text(
		pre_settings, WarningsManager.create_warnings_dictionary_from_project_settings(), 0
	)
...
func _diff(name_1, name_2):
	if warning_settings.has(name_1) and warning_settings.has(name_2):
		...
		var heading = str(
			" ".repeat(WARN_VALUE_PRINT_POSITION), name_1.rpad(c2_pad, " "), name_2, "\n"
		)
		heading += str(
...
func _setup_options():
	...
	opts.add(
		...
		[],
		(
			"Sets a single setting in the project settings and saves.\n"
	...
	opts.add(
		...
		[],
		(
			"Shows the difference between two Warning Level Names.\n"
	...
	(
		...
		. add(
			"-apply",
			"none",
			"Applys one of the Warning Level Names to the project settings.  You should restart after using this"
		)
	)
...
func _init():
	...
	if opts.values.h:
		...
		opts.print_help()
		print(
			"You didn't specify any options or too many or not the right size or something invalid.  I don't know what you want to do."
		)
